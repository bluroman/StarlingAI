...
public class SteeredVehicle extends Vehicle
	{
		...
		private var _wanderDistance:Number = 100;
        ...
        public function avoidBubbles(bubbles:Array):void
        {
            var bubble_position:Vector2D = new Vector2D();
            for(var i:int = 0; i < bubbles.length; i++)
            {
                var bubble:Image = bubbles[i] as Image;
                var heading:Vector2D = _velocity.clone().normalize();

                // vector between circle and vehicle:
                bubble_position.x = bubble.x;
                bubble_position.y = bubble.y;
                var difference:Vector2D = bubble_position.subtract(_position);
                ...
                if(dotProd > 0)
                {
                    ...
                    if(dist < bubble.width + _avoidBuffer &&
